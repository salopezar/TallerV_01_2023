...
void extInt_Config(EXTI_Config_t *extiConfig){
	...
	case 2: {
		/* SYSCFG_EXTICR1 */
		//Limpiamos la posición que queremos configurar
	...
	if(extiConfig->edgeType == EXTERNAL_INTERRUPT_FALLING_EDGE){
		...
		EXTI->FTSR |= (1 << extiConfig->pGPIOHandler->GPIO_PinConfig.GPIO_PinNumber);
	}
	else{
		EXTI->RTSR |= (1 << extiConfig->pGPIOHandler->GPIO_PinConfig.GPIO_PinNumber);
	}
...
void EXTI9_5_IRQHandler(void){
	...
	if(EXTI->PR & EXTI_PR_PR5){
		// Se baja la bandera correspondiente
		EXTI->PR |= EXTI_PR_PR5;
